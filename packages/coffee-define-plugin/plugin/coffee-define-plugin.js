...
var define = function(str) {
	var defineRegExp =/[\ \t]*#define\s+([a-zA-Z]+\w*|\_+[a-zA-Z]+\w*)\s(.*)\n/gm;
  var defines = new Array();
	var m;
  var index = 0;

	while ((m = defineRegExp.exec(str)) !== null) {
    defines[index] = {};
    defines[index].string = m[0];
    defines[index].key = m[1];
    defines[index].val = m[2];
    index++;
  }

  for (var i = 0; i < defines.length; i++) {
    var key = defines[i].key;
    var val = defines[i].val
    str = str.replace(defines[i].string, "");
    str = str.replace(new RegExp(key, "gm"), 
      function(match, offset, string) {
        if (/\W/.test(string[offset - 1]) 
          && (/\W/.test(string[offset + match.length])
            || offset + match.length == string.length)) {
          return match.replace(key, val);
        }
        return match;
    }); 
  }
